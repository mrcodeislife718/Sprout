...
export function signal(initialValue) {
  ...
  return {
    ...
    update(updater) { this.value = updater(value); return value; },
...
export function computed(read, dependencies = []) {
  ...
  return { get value() { return output.value; }, subscribe: output.subscribe, dispose() { unsubscribers.forEach((unsubscribe) => unsubscribe()); } };
}

export function effect(run, dependencies = []) {
  if (typeof run !== 'function') throw new TypeError('effect(run) expects a function');
  let cleanup;
  const execute = () => { if (typeof cleanup === 'function') cleanup(); cleanup = run(); };
  execute();
...
function createDomNode(vnode, documentRef) {
  ...
  if (vnode.type === Symbol.for('sprout.fragment')) { const fragmentNode = documentRef.createDocumentFragment(); for (const child of vnode.children) fragmentNode.appendChild(createDomNode(child, documentRef)); return fragmentNode; }
