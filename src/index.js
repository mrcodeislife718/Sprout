...
class ReactiveObserver {
  ...
  depend(source) {
    if (this.disposed || this.dependencies.has(source)) return;
    const unsubscribe = source.subscribe(() => this.schedule());
    this.dependencies.set(source, unsubscribe);
  }
  ...
  execute() {
    ...
    if (this.cleanupResult && typeof this.cleanup === 'function') {
    ...
    try {
      ...
      if (this.cleanupResult) this.cleanup = typeof result === 'function' ? result : null;
  ...
  schedule() {
    if (this.disposed) return;
    if (this.running) { this.pending = true; return; }
    if (batchDepth > 0) { pendingObservers.add(this); return; }
    this.execute();
  }
  ...
  dispose() {
    ...
    for (const unsubscribe of this.dependencies.values()) unsubscribe();
    this.dependencies.clear();
    if (this.cleanupResult && typeof this.cleanup === 'function') this.cleanup();
    this.cleanup = null;
  }
}
...
export function h(type, props = {}, ...children) {
  ...
  return { type, props: props ?? {}, children: flat.map(normalizeChild) };
}

export const fragment = (...children) => h(Symbol.for('sprout.fragment'), {}, ...children);

function normalizeChild(value) {
  if (typeof value === 'string' || typeof value === 'number' || typeof value === 'bigint') return { type: TEXT, value: String(value), props: {}, children: [] };
  return value;
}
...
export function signal(initialValue) {
  ...
  const api = {
    ...
    peek() { return value; },
    update(updater) {
      if (typeof updater !== 'function') throw new TypeError('signal.update expects a function');
...
export function computed(read, dependencies = null) {
  ...
  if (dependencies !== null) {
    const list = normalizeDependencies(dependencies);
    output.value = untracked(read);
    const recompute = () => { output.value = untracked(read); };
    const unsubscribers = list.map((dependency) => dependency.subscribe(recompute));
    return {
  ...
  const observer = new ReactiveObserver(read, { onResult: (value) => { output.value = value; } });
  observer.execute();
  return {
    get value() { return output.value; },
    peek: output.peek,
...
export function effect(run, dependencies = null) {
  ...
  if (dependencies !== null) {
    ...
    const execute = () => {
      ...
      cleanup = untracked(run);
    };
    execute();
    const unsubscribers = list.map((dependency) => dependency.subscribe(execute));
    return () => {
      unsubscribers.forEach((unsubscribe) => unsubscribe());
      if (typeof cleanup === 'function') cleanup();
    };
  }
  const observer = new ReactiveObserver(run, { cleanupResult: true });
  observer.execute();
  return () => observer.dispose();
}

export function batch(work) {
  if (typeof work !== 'function') throw new TypeError('batch(work) expects a function');
  batchDepth += 1;
  try { return work(); }
  finally {
    batchDepth -= 1;
...
export function untracked(work) {
  if (typeof work !== 'function') throw new TypeError('untracked(work) expects a function');
  const previous = activeObserver;
  activeObserver = null;
  try { return work(); }
...
function flushPendingObservers() {
  while (pendingObservers.size) {
    const observers = [...pendingObservers];
    pendingObservers.clear();
    for (const observer of observers) observer.execute();
  }
}

function normalizeDependencies(dependencies) {
  if (!Array.isArray(dependencies)) throw new TypeError('dependencies must be an array or null');
  for (const dependency of dependencies) if (!dependency || typeof dependency.subscribe !== 'function') throw new TypeError('reactive dependency must expose subscribe()');
...
function applyProps(element, props) {
  for (const [key, value] of Object.entries(props ?? {})) {
    ...
    if (key === 'style' && value && typeof value === 'object') { for (const [property, styleValue] of Object.entries(value)) element.style[property] = styleValue; continue; }
    if (key.startsWith('on') && typeof value === 'function') { element.addEventListener(key.slice(2).toLowerCase(), value); continue; }
    if (key in element && typeof value !== 'object') { try { element[key] = value; continue; } catch {} }
    if (value === true) element.setAttribute(key, ''); else element.setAttribute(key, String(value));
  }
}
...
export function renderToString(vnode) {
  ...
  const attributes = Object.entries(vnode.props ?? {}).filter(([key, value]) => key !== 'children' && !key.startsWith('on') && value != null && value !== false).map(([key, value]) => serializeAttribute(key, value)).filter(Boolean).join(' ');
